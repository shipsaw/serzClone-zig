...
pub const status = struct {
    ...
    activeParent: std.ArrayList(*node),
    ...
    pub fn init(src: []const u8) !status {
        return status{
            ...
            .activeParent = std.ArrayList(*node).init(allocator),
            .result = null,
        };
    }
...
pub fn parse(s: *status) !std.ArrayList(node) {
    ...
    const rootNode = processFF50(s);
    s.result = rootNode;
    try s.activeParent.put(&rootNode);
...
fn processFF50(s: *status) !ff50node {
    ...
    var newNode = ff50node{
        ...
        .children = std.ArrayList(node).initCapacity(numChildren),
    };
    s.activeParent = &newNode;
    return newNode;
}
...
//     // Arrange
//     var testStatus = try status.init("Hello");
//
//     // Act
