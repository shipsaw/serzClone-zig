        ...
        if (s.source[s.current] == 0xff) {
            ...
            switch (s.source[s.current]) {
                0x41 => {
                    ...
                    try currentParent.children.append(node{ .ff41node = tok });
                    try s.savedTokenList.append(node{ .ff41node = tok });
                },
                0x4e => {
                    s.current += 1;
                    try currentParent.children.append(node{ .ff4enode = ff4enode{} });
                ...
                0x56 => {
                    ...
                    try currentParent.children.append(node{ .ff56node = tok });
                    try s.savedTokenList.append(node{ .ff56node = tok });
                },
                0x70 => {
                    s.current += 1;
                    const tok = try processFF70(s);
                    try currentParent.children.append(node{ .ff70node = tok });
                    try s.savedTokenList.append(node{ .ff70node = tok });
                },
                else => return errors.InvalidNodeType,
            }
        } else {
            try currentParent.children.append(try processSavedLine(s));
        }
...
test "ff50 parsing" {
    ...
    const expected = ff50node{ .name = "food", .id = 375192228, .numChildren = 1, .children = std.ArrayList(node).init(allocator) };
...
//         node{ .ff50node = ff50node{ .name = "first", .id = 375192228, .numChildren = 1, .children = std.ArrayList(node).init(allocator) } },
...
test "parse function" {
    ...
    var expected = ff50node{ .name = "first", .id = 375192228, .numChildren = 1, .children = std.ArrayList(node).init(allocator) };
    ...
    // Assert
    try expectEqualStrings(result.name, expected.name);
    try expect(result.id == expected.id);
    try expect(result.numChildren == expected.numChildren);

    try expectEqualStrings(result.children.items[0].ff56node.name, expected.children.items[0].ff56node.name);
